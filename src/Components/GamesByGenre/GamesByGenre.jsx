...
const GamesByGenre= ({genreId, mode}) => {
    ...
    useEffect(() => {
        fetch(`https://api.rawg.io/api/games?key=4bc0eac8b3e74a84a29fa89b0d4181a8&genres=${genreId}`)
        ...
        <>
        {
          ...
          gamesByGenre.map( gameByGenre => {
                return      <div className={`items ${mode === false ? 'category-l-mode' : 'category-d-mode'}`}>
                 <Link key={gameByGenre.id}
                    to={`/game-description/${gameByGenre.slug}`}
                    >
                    <img src={gameByGenre.background_image} alt="game-image" loading="lazy" />
