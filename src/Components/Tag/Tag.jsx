...
const Tag = ({mode}) => {
   ...
   useEffect(() => {
    fetch(`https://api.rawg.io/api/games?key=4bc0eac8b3e74a84a29fa89b0d4181a8&tags=${tagId}`)
        ...
        <div className={`container ${mode === false ? 'l-mode' : 'd-mode'}`}>
            <h1>{tagId[0].toUpperCase() + tagId.substring(1)} Games</h1>
            {
              ...
              gamesByTag.map(gameByTag => {
                    return <Link key={gameByTag.id} to={`/game-description/${gameByTag.slug}`} className='card'>
                    <img src={gameByTag.background_image} alt="game-image" loading="lazy"/>
