...
const GamesByPublisher = ({ publisherId, mode }) => {
  ...
  useEffect(() => {
    fetch(
      `https://api.rawg.io/api/games?key=4bc0eac8b3e74a84a29fa89b0d4181a8&publishers=${publisherId}`
    )
      ...
      {gamesByPublisher.length === 0 ? (
        ...
        gamesByPublisher.map((gameByPublisher) => {
          return (
            ...
            >
              <Link
                key={gameByPublisher.id}
                to={`/game-description/${gameByPublisher.slug}`}
              >
                <img
                  src={gameByPublisher.background_image}
                  alt="game-img"
                  loading="lazy"
                />
